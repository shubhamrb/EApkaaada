...
int selectValue=0;
    public SubCatAdapter(ArrayList<SubCategory> list, OnClickListener listener, Context context) {
        this.list = list;
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        SubCategory data = list.get(position);
        holder.tvName.setText(data.getName());

        holder.cardView.setOnClickListener(v -> {
            selectValue=position;
            onClickListener.catOnClick(data, position, "");
    ...
    public interface OnClickListener {
        void catOnClick(SubCategory data, int pos, String type);
    }
