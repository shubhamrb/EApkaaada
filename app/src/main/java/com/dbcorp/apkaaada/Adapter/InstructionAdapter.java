...
public class InstructionAdapter extends RecyclerView.Adapter<InstructionAdapter.MyViewHolder> {
    ...
    int pos=8;
    public InstructionAdapter(ArrayList<Instruction> list, OnClickListener listener, Context context) {
        this.list = list;
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        Instruction data=list.get(position);
        holder.tvName.setText(data.getName());
        holder.card_view.setOnClickListener(v->{

            pos=position;
            onClickListener.onClickCard(list,position,"");
    ...
    public interface OnClickListener{
        void onClickCard(ArrayList<Instruction> list, int pos, String type);
    }
